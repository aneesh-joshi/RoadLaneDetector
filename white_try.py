...
for i in range(8):
    ...
    combined_right = np.zeros_like(dir_binary)
    ...
    combined_left = np.zeros_like(dir_binary)
    ...
    mpimg.imsave('thresholded_images/test' + str(i+1), combined_final)

    white = mag_thresh(hsv[:,:,2], mag_thresh=(200, 255))

    axes[0, 0].set_title('L_uv')
    axes[0, 0].imshow(combined_final, cmap='gray')

    axes[0, 1].set_title('R')
    axes[0, 1].imshow(image[:,:,0], cmap='gray')

    axes[0, 2].set_title('hs_V')
    ...
    axes[1, 0].set_title('gradxL')
    axes[1, 0].imshow(gradx_Luv, cmap='gray')

    axes[1, 1].set_title('mag S')
    axes[1, 1].imshow(mag_r, cmap='gray')
